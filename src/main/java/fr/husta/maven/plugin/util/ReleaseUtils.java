...
public class ReleaseUtils
{
     ...
     */
    public static String getReleaseVersion(String pomVersion) throws MojoExecutionException
    {
        String releaseVersion = null;
        // Remove "-SNAPSHOT" from the end, if it's there
        if (pomVersion != null && pomVersion.endsWith(SNAPSHOT_SUFFIX))
        {
            releaseVersion = pomVersion.substring(0, pomVersion.length() - SNAPSHOT_SUFFIX.length());
        }
        ...
        if (releaseVersion == null)
        {
            throw new MojoExecutionException(
                    "Couldn't find the release '" + pomVersion + "' among the supplied version.");
        }
